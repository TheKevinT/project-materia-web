...
var controller = {
    ...
    getProjects: function(req, res) {
        ...
        //ordenar por año de mayo a menor 

        // if (projectId == null) return res.status(404).send({ message: 'El proyecto no existe' })

        //condicion para ordenarlo por año

        // Project.find({ year: 2020 }).exec((err, project) => {
    ...
    uploadImage: function(req, res) {
        ...
        if (req.files) {
            ...
            var fielExt = extSplit[1];

            if (fielExt == 'png' || fielExt == 'jpg' || fielExt == 'jpeg') {
                Project.findByIdAndUpdate(projecId, { image: fileName }, { new: true }, (err, projectUpdate) => {

                    if (err) return res.status(500).send({ message: 'La imagen no se ha subido' });
                    if (!projectUpdate) return res.status(404).send({ message: 'El proyecto no existe y  no se subio la imagen' });
                    return res.status(200).send({ project: projectUpdate });
